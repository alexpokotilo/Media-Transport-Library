...
class RxTxApp(Application):
    ...
    def create_command(self, **kwargs):
        ...
        # Every session's own params, for integrity_intents(); empty when single.
    ...
    def integrity_intents(self, test_repo_path: str, host) -> list[IntegrityIntent]:
        """One intent per st20p and st30p session of a ``sessions=[...]`` run.

        No other session type is byte-checked: st22p is lossy, and the rest
        have no checker. Each intent is built with its own session's params in
        place, so it names that session's files.
        """
        if not self._session_params:
            return super().integrity_intents(test_repo_path, host)
        primary, intents = self.params, []
        try:
            for params in self._session_params:
                if params["session_type"] in ("st20p", "st30p"):
                    self.params = params
